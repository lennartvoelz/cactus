...
void Qwen3p5MoEModel::post_init() {
    ...
    // DeltaNet dimension inference — same logic as Qwen3p5Model
    ...
    if (has_deltanet_layer &&
        ...
        (void)static_cast<size_t>(config_.linear_num_key_heads);
        ...
        if (num_v_heads == 0) {
            throw std::runtime_error("Qwen3p5MoEModel invalid linear-attention value head count");
        }
        if (value_head_dim == 0) {
            if (config_.linear_v_proj_dim > 0 && (config_.linear_v_proj_dim % num_v_heads == 0)) {
                ...
                throw std::runtime_error("Qwen3p5MoEModel cannot derive linear-attention value head dim");
            }
        }
        ...
        if (!gb || !first_linear) {
            throw std::runtime_error("Qwen3p5MoEModel cannot infer linear-attention dims");
        }
        ...
        if (q_buf.shape.size() < 2 || v_buf.shape.size() < 2 || n_buf.shape.empty()) {
            throw std::runtime_error("Qwen3p5MoEModel invalid linear-attention weight shapes");
        }
        ...
        if (inferred_key_dim == 0 || q_proj_dim == 0 || v_proj_dim == 0) {
            throw std::runtime_error("Qwen3p5MoEModel failed to infer non-zero linear-attention dims");
        }
        if (q_proj_dim % inferred_key_dim != 0) {
            throw std::runtime_error("Qwen3p5MoEModel q projection dim not divisible by inferred key dim");
        }
        ...
        if (inferred_k_heads == 0 || inferred_v_heads == 0 || inferred_v_heads % inferred_k_heads != 0) {
            throw std::runtime_error("Qwen3p5MoEModel failed to infer compatible linear-attention head topology");
        }
    ...
    if (deltanet_heads_ == 0 || deltanet_key_dim_ == 0 || deltanet_value_dim_ == 0) {
        throw std::runtime_error("Qwen3p5MoEModel requires non-zero deltanet heads/key/value dims");
    }
    ...
    if (config_.linear_q_proj_dim > 0 && config_.linear_v_proj_dim > 0) {
        ...
        auto* gb = static_cast<CactusGraph*>(graph_handle_);
        if (!gb || !first_linear) {
            throw std::runtime_error("Qwen3p5MoEModel cannot infer linear-attention mixed projection dim");
        }
        const auto& q_buf = gb->get_output_buffer(first_linear->attn_q_weight);
        const auto& v_buf = gb->get_output_buffer(first_linear->attn_v_weight);
        if (q_buf.shape.size() < 2 || v_buf.shape.size() < 2) {
            throw std::runtime_error("Qwen3p5MoEModel invalid q/v projection shapes for mixed dim inference");
        }
    ...
    if (has_deltanet_layer) {
        auto* gb = static_cast<CactusGraph*>(graph_handle_);
        if (!gb) {
            throw std::runtime_error("Qwen3p5MoEModel cannot infer conv history: graph unavailable");
        }
        for (const auto& layer : weight_nodes_.layers) {
            ...
            if (conv_buf.shape.empty()) continue;
    ...
    if (deltanet_conv_history_len_ > 0 && deltanet_mixed_dim_ == 0) {
        throw std::runtime_error("Qwen3p5MoEModel linear-attention conv history requires non-zero mixed dim");
    }
    ...
    if (deltanet_cache_row_dim_ == 0) {
        throw std::runtime_error("Qwen3p5MoEModel computed zero cache row dim");
    }
...
void Qwen3p5MoEModel::reset_cache() {
    Model::reset_cache();
...
void Qwen3p5MoEModel::post_execute_updates(CactusGraph* gb, size_t seq_len) {
...
void Qwen3p5MoEModel::load_weights_to_graph(CactusGraph* gb) {
    ...
    auto mmap_required = [&](const std::vector<std::string>& candidates, const std::string& weight_name) -> size_t {
        ...
        if (path.empty()) {
            throw std::runtime_error("Qwen3p5MoEModel missing required weight: " + weight_name);
        }
        return gb->mmap_weights(path);
    };

    auto mmap_optional = [&](const std::vector<std::string>& candidates) -> size_t {
        const std::string path = choose_existing_weight(candidates);
    ...
    for (uint32_t i = 0; i < config_.num_layers; i++) {
        ...
        std::string lp = model_folder_path_ + "/layer_" + std::to_string(i) + "_";
        ...
        // Attention / linear-attention weights (same as Qwen3p5Model)
        layer.attn_q_weight = mmap_required(
            {lp + "attn_q.weights", lp + "linear_attn_q.weights"},
            "layer_" + std::to_string(i) + "_attn_q");
        layer.attn_k_weight = mmap_required(
            {lp + "attn_k.weights", lp + "linear_attn_k.weights"},
            "layer_" + std::to_string(i) + "_attn_k");
        layer.attn_v_weight = mmap_required(
            {lp + "attn_v.weights", lp + "linear_attn_v.weights"},
            "layer_" + std::to_string(i) + "_attn_v");
        layer.attn_output_weight = mmap_required(
            {lp + "attn_output.weights", lp + "linear_attn_output.weights"},
            "layer_" + std::to_string(i) + "_attn_output");
        layer.input_layernorm_weight = mmap_required(
            {lp + "input_norm.weights"},
            "layer_" + std::to_string(i) + "_input_norm");
        layer.attn_q_norm_weight = mmap_required(
            {lp + "attn_q_norm.weights", lp + "linear_attn_norm.weights"},
            "layer_" + std::to_string(i) + "_attn_q_norm");
        layer.attn_k_norm_weight = mmap_optional(
            {lp + "attn_k_norm.weights", lp + "linear_attn_norm.weights"});

        if (is_deltanet) {
            layer.deltanet_qkv_weight = mmap_optional({lp + "linear_attn_qkv.weights"});

            const std::string gate_path = choose_existing_weight({
                lp + "deltanet_gate.weights", lp + "attn_gate.weights",
                lp + "attn_f_gate.weights", lp + "linear_attn_a.weights"
            });
            const std::string beta_path = choose_existing_weight({
                lp + "deltanet_beta.weights", lp + "attn_beta.weights",
                lp + "attn_f_beta.weights", lp + "linear_attn_b.weights"
            });
            if (gate_path.empty() || beta_path.empty()) {
                throw std::runtime_error(
                    "Qwen3p5MoEModel deltanet layer missing gate/beta weights at layer " + std::to_string(i));
            }
            layer.deltanet_gate_weight = gb->mmap_weights(gate_path);
            layer.deltanet_beta_weight = gb->mmap_weights(beta_path);

            layer.deltanet_gate_bias = mmap_optional({lp + "linear_attn_A_log.weights"});
            layer.deltanet_beta_bias = mmap_optional({lp + "linear_attn_dt_bias.weights"});
            layer.deltanet_z_weight = mmap_optional({lp + "linear_attn_z.weights"});
            layer.deltanet_conv_weight = mmap_optional({lp + "linear_attn_conv1d.weights"});
        }

        layer.post_attention_layernorm_weight = mmap_required(
            {lp + "post_attn_norm.weights"},
            "layer_" + std::to_string(i) + "_post_attn_norm");

        // MoE weights
        layer.moe_router_weight = mmap_required(
            {lp + "moe_router.weights"},
            "layer_" + std::to_string(i) + "_moe_router");
        ...
        for (uint32_t expert_idx = 0; expert_idx < config_.num_experts; ++expert_idx) {
            ...
            std::string ep = lp + "moe_expert_" + std::to_string(expert_idx) + "_";
            expert.w1_weight = mmap_required({ep + "w1.weights"},
                "layer_" + std::to_string(i) + "_moe_expert_" + std::to_string(expert_idx) + "_w1");
            expert.w3_weight = mmap_required({ep + "w3.weights"},
                "layer_" + std::to_string(i) + "_moe_expert_" + std::to_string(expert_idx) + "_w3");
            expert.w2_weight = mmap_required({ep + "w2.weights"},
                "layer_" + std::to_string(i) + "_moe_expert_" + std::to_string(expert_idx) + "_w2");
        }

        // Shared expert weights (optional — some layers may not have them)
        layer.shared_expert_gate_weight = mmap_optional({lp + "shared_expert_gate.weights"});
        layer.shared_expert_up_weight = mmap_optional({lp + "shared_expert_up.weights"});
        layer.shared_expert_down_weight = mmap_optional({lp + "shared_expert_down.weights"});
        layer.shared_expert_gate_proj_weight = mmap_optional({lp + "shared_expert_gate_proj.weights"});
    }
}

// Gated DeltaNet — identical to Qwen3p5Model::build_gated_deltanet
size_t Qwen3p5MoEModel::build_gated_deltanet(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
    ...
    if (in_shape.size() < 2) {
        throw std::runtime_error("Qwen3p5MoEModel linear-attention expects rank-2 normalized input");
    }
    ...
    if (layer.deltanet_qkv_weight != 0) {
        ...
        if (mixed_shape.size() < 2 || mixed_shape[0] != seq_len) {
            throw std::runtime_error("Qwen3p5MoEModel invalid joint qkv projection shape");
        }
        if (mixed_shape[1] <= v_proj_dim || ((mixed_shape[1] - v_proj_dim) % 2) != 0) {
            throw std::runtime_error("Qwen3p5MoEModel invalid joint qkv projection width");
        }
        ...
        if (q_shape.size() < 2 || q_shape[0] != seq_len) {
            throw std::runtime_error("Qwen3p5MoEModel invalid q projection shape");
        }
    ...
    if (qk_proj_dim == 0 || qk_proj_dim % key_dim != 0 || v_proj_dim == 0) {
        throw std::runtime_error("Qwen3p5MoEModel invalid linear-attention projection dimensions");
    }
    const size_t num_k_heads = qk_proj_dim / key_dim;
    if (num_heads % num_k_heads != 0) {
        throw std::runtime_error("Qwen3p5MoEModel requires num_v_heads divisible by num_k_heads");
    }
    ...
    if (deltanet_mixed_dim_ != 0 && mixed_proj_dim != deltanet_mixed_dim_) {
        throw std::runtime_error("Qwen3p5MoEModel linear-attention mixed projection dim mismatch");
    }
        ...
        if (prev_state_flat == 0) {
            ...
            if (!segments.empty()) {
                ...
                for (size_t s = 1; s < segments.size(); ++s) {
                    stacked = gb->concat(stacked, segments[s], 0);
                }
                if (view.total_len > 1) {
    ...
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_deltanet_a_logits", a_logits);
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_deltanet_b_logits", b_logits);
    size_t a_softplus = gb->scalar_log(gb->scalar_add(gb->scalar_exp(a_logits), 1.0f));
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_deltanet_a_softplus", a_softplus);
    ...
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_deltanet_gate_log", gate_log);
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_deltanet_beta", beta);
    ...
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_deltanet_out", deltanet_out);
...
size_t Qwen3p5MoEModel::build_attention(CactusGraph* gb, size_t normalized_input, uint32_t layer_idx,
                                         ComputeBackend backend, bool use_cache, size_t position_offset) {
    const auto& layer_entry = weight_nodes_.layers[layer_idx];
    const auto& layer = layer_entry.weights;
    ...
    // Full attention — same as Qwen3p5Model
    ...
    size_t k_norm_weight_node = layer.attn_k_norm_weight;
    if (k_norm_weight_node != 0) {
        k_norm_weight_node = gb->scalar_add(k_norm_weight_node, 1.0f);
    } else {
        k_norm_weight_node = q_norm_weight;
    }
    k_proj = gb->reshape(k_proj, {batch_seq * num_kv_heads, head_dim});
    k_proj = gb->rms_norm(k_proj, k_norm_weight_node, config_.layer_norm_eps);
    k_proj = gb->reshape(k_proj, {batch_seq, num_kv_heads * head_dim});
    ...
    if (config_.rope_theta > 0) {
        ...
        if (rotary_factor <= 0.0f) rotary_factor = 0.25f;
        size_t rot_dim = static_cast<size_t>(static_cast<float>(head_dim) * rotary_factor);
        rot_dim = std::min(rot_dim, head_dim);
        if ((rot_dim & 1u) != 0) rot_dim -= 1;

        auto apply_partial_rope = [&](size_t x_4d) -> size_t {
            if (rot_dim == 0) return x_4d;
            if (rot_dim >= head_dim) return gb->rope(x_4d, config_.rope_theta, position_offset);
    ...
    if (use_cache && !kv_cache_.is_empty() && kv_cache_.precision == Precision::INT8) {
        // INT8 KV cache: use dedicated hybrid kernel that dequantizes on the fly
        ...
        // FP16 KV cache: load cached K/V as graph inputs, concat with new, run standard attention
        ...
        size_t final_k = gb->concat(cache_k_node, k_proj_4d, 1);
        size_t final_v = gb->concat(cache_v_node, v_proj_4d, 1);
        ...
        // No cache or cache empty: standard attention on current tokens only
        if (use_cache) {
            cache_k_output_nodes_[layer_idx] = k_proj_4d;
            cache_v_output_nodes_[layer_idx] = v_proj_4d;
        }
...
size_t Qwen3p5MoEModel::build_mlp(CactusGraph* gb, size_t normalized_h, uint32_t layer_idx,
                                   ComputeBackend backend) const {
    const auto& layer = weight_nodes_.layers[layer_idx].weights;

    // Softmax router → top-k expert selection
    ...
    auto topk_indices_and_values = gb->topk(routing_probs, config_.num_experts_per_tok);
    ...
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_moe_routing_probs", routing_probs);
    size_t moe_output = gb->moe_layer(
        normalized_h,
        routing_probs,
        topk_indices,
        w1_weights,
    ...
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_moe_output", moe_output);
    // Shared expert path: SwiGLU FFN gated by sigmoid
    if (layer.shared_expert_gate_weight != 0 &&
        layer.shared_expert_up_weight != 0 &&
        layer.shared_expert_down_weight != 0) {
        size_t se_gate = gb->matmul(normalized_h, layer.shared_expert_gate_weight, true, backend);
        size_t se_up = gb->matmul(normalized_h, layer.shared_expert_up_weight, true, backend);
        size_t se_activated = gb->multiply(gb->silu(se_gate), se_up);
        size_t se_down = gb->matmul(se_activated, layer.shared_expert_down_weight, true, backend);

        // Apply sigmoid gate if present
        if (layer.shared_expert_gate_proj_weight != 0) {
            size_t gate_logit = gb->matmul(normalized_h, layer.shared_expert_gate_proj_weight, true, backend);
            size_t gate_value = gb->sigmoid(gate_logit);
            se_down = gb->multiply(se_down, gate_value);
        }

        moe_output = gb->add(moe_output, se_down);
    }

    return moe_output;
}

size_t Qwen3p5MoEModel::build_transformer_block(CactusGraph* gb, size_t hidden, uint32_t layer_idx,
                                                 ComputeBackend backend, bool use_cache, size_t position_offset) {
    const auto& layer = weight_nodes_.layers[layer_idx].weights;
    ...
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_after_attn_residual", after_attention);
    ...
    size_t block_output = gb->add(after_attention, mlp_output);
    capture_debug_node(layer_idx, "L" + std::to_string(layer_idx) + "_block_output", block_output);
    return block_output;
}

size_t Qwen3p5MoEModel::forward(const std::vector<uint32_t>& tokens, bool use_cache) {
    ...
    capture_debug_node(99, "embedding_output", hidden);
    ...
    capture_debug_node(99, "final_hidden", final_hidden);
    return final_hidden;
}
