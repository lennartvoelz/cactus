...
def get_layer_weight_patterns(i, precision, model_type=None, skip_kv=False):
    ...
    patterns = [
        ...
        (['feed_forward.gate.weight', 'mlp.gate.weight'], precision, f'layer_{i}_moe_router.weights', False),
        ...
        (['mlp.experts.gate_up_proj'], precision, f'layer_{i}_moe_packed_gate_up.weights', False) if is_qwen_family else None,
        (['mlp.experts.down_proj'], precision, f'layer_{i}_moe_packed_down.weights', False) if is_qwen_family else None,
        (['mlp.shared_expert.gate_proj.weight'], precision, f'layer_{i}_shared_expert_gate.weights', False) if is_qwen_family else None,
        (['mlp.shared_expert.up_proj.weight'], precision, f'layer_{i}_shared_expert_up.weights', False) if is_qwen_family else None,
        (['mlp.shared_expert.down_proj.weight'], precision, f'layer_{i}_shared_expert_down.weights', False) if is_qwen_family else None,
        (['mlp.shared_expert_gate.weight'], precision, f'layer_{i}_shared_expert_gate_proj.weights', False) if is_qwen_family else None,
